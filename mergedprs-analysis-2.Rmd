...
We can see there is an outlier with 46 non-trivial partitions. This pull request consists of a refactoring where the return type of several methods was changed and test cases were added for these methods. Due to the limitations of ClusterChanges, a lot of partitions were generated for this pull request. To better visualize the data, we limit the x axis to a maximum of 15 non-trivial partitions (only this outlier has more than 15 non-trivial partitions).
...
ggplot(summ, aes(x=nonTrivialPartitions, y=..density.., fill=merged)) +
  ...
  xlim(c(0, 15)) +
  facet_wrap(~merged, nrow = 2) +
  ggtitle("Histogram of Non-Trivial Partitions in a Pull Request (zoomed)") +
  xlab("Non-Trivial Partitions") +
  ylab("Proportion (in-group) of Pull Requests") +
  labs(fill = "Merged")
```
...
At first glance, there is an obvious outlier with 394 trivial partitions, which is the same one with 46 non-trivial partitions. To better visualize the data, we limit the x axis to a maximum of 32 trivial partitions (only 4 pull requests have more than 31 trivial partitions).
...
**TODO:** Update this to reflect new sample results!

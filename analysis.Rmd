...
The distribution of diff-regions is strongly positively skewed. While `r percent(diffRegionsECDF(1))` of the pull requests have at most 1 diff-region and therefore are likely trivial to understand, as much as `r percent(1 - diffRegionsECDF(10))` pull requests have at least 10 diff-regions and 25% of the pull requests have at least `r diffRegionsQuantiles["75%"]` diff-regions. These large pull requests likely contain independent modifications and are hard to understand.
...
Our manual analysis also allowed us to learn more about what relationships ClusterChanges does not capture. We observed that a very common type of diff-region that is never grouped with any others is the sequence of package declaration and import statements before a type declaration at the beginning of Java source code files. This happens because ccjava does not consider such statements as definitions or uses. Almost all pull requests have diff-regions of this type and these end up in separate trivial partitions. Diff-regions composed entirely of blank lines, comments and Javadoc comments are also frequent occurrences and each of such diff-regions also gets assigned it's own trivial partition.
...
We manually analyzed a random sample of 15 out of `r nrow(filter(summ, trivialPartitions > 10))` pull requests with more than 10 trivial partitions. Considering it unlikely for a pull request to contain so many separate changes, we wanted to know more about the relationships that ccjava does not capture.
